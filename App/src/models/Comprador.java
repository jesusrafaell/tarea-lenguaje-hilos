 ...
 // state or field
  ...
  public void run() {
    ...
    try{
      ...
      System.out.println(
        "Hilo Comprador: " + name + " esperara " + time
      );
      Thread.sleep(time);
      System.out.println(
        "Inicio Hilo Comprador: " + name 
      );
    ...
    while(!exit){
      try {
        ...
        for (int i = 0; i < subastas.size(); i++) {
          ...
          if (
            montoSubastar <= this.monto && 
            this != subastas.get(i).getComprador() &&
            checkPuja(subastas.get(i))
          ) {
            System.out.println(
              "Hilo Comprador: " + name +
